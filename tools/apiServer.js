...
server.post("/deliveryBoys/", function (req, res, next) {
  const error = validateDeliveryBoy(req.body);
  if (error) {
...
function validateDeliveryBoy(deliveryBoy) {
  if (!deliveryBoy.name) return "Name is required.";
  if (!deliveryBoy.managerId) return "Manager is required.";
  if (!deliveryBoy.category) return "Category is required.";
  return "";
}
